...
function getMoreBooks(txt) {
    // if (localStorage.getItem(USERS)) {
    //     return new Promise(res => {
    //         res(JSON.parse(localStorage.getItem(USERS)))
    //     })
    // }
    return axios.get(`https://www.googleapis.com/books/v1/volumes?q=${txt}:keyes&key=AIzaSyC2uhgNiCxFUJjJWr892yeqlA4FjaAhD9g`)
        .then(res => {
            var books = res.data.items
            return books
                //     var reposCount = JSON.parse(getReposOfUser(user.repos_url))
...
function createBook(book) {
    ...
    booksDBNew.push(newBook)
    storageService.store(BOOKS_KEY, booksDBNew)
...
// AIzaSyC2uhgNiCxFUJjJWr892yeqlA4FjaAhD9g
...
function getBookById(id) {
    createNewDB()
    return Promise.resolve(booksDB.find(book => book.id === id))
}
